...
const BASE_URL = "http://127.0.0.1:8086/api/v1/";
@Injectable({
  providedIn: 'root',
})
export class AuthService {
  ...
  login(auth: AuthenticationRequest): Observable<AuthenticationResponse> {
    console.log(auth);
    return this.http.post<AuthenticationResponse>('http://localhost:8085/login', auth);
  }

  register(data: RegisterRequest): Observable<AuthenticationResponse> {
    return this.http.post<AuthenticationResponse>('http://localhost:8085/login', data);
  }
